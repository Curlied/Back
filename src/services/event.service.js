...
const getAll = async () => {
  return await Event.find({
    is_validate: true,
    date_time: { $gte: Date.now() },
  });
};

const getAllPaginate = async (page = 0, limit = 10) => {
  return await Event.paginate(
    { is_validate: true, date_time: { $gte: Date.now() } },
    { page, limit }
  );
};
...
const submitParticipant = async (user_id, event_id) => {
  const userParticipate = {
    user_id: user_id,
    status: constantes.STATUS_EVENT.VALIDATE,
  };
...
const findAllForSpaceUserByCreator = async (creator_id) => {
  ...
  return await Event.find(filter).select(
    'category date_time place name url_image users_valide users_waiting users_refused users_cancel user_max is_validate'
  );
};
...
const findAllEventsUserParticpateIn = async (user_id) => {
  return await Event.find({
    ...
    'category date_time place name url_image users_valide users_waiting user_max'
  );
};
...
const findAllByCreatorForMyProfil = async (_idCreator) => {
  ...
  return await Event.find(filter).select(
    'category date_time place name url_image'
  );
};
...
const IsUserParticipeOnEvent = async (request) => {
  ...
  const {
    token: { userId },
  } = await retrieve_user_from_token(getHeaderToken(request));
...
const hasPlaceToParticipeOnEvent = async (event_id) => {
  ...
  return event.users_waiting.length + event.users_valide.length < event.user_max
    ? true
    : false;
};
...
const searchEvents = async (request) => {
  const { body } = request;
  const { category, department, date, element, search, page, limit } = body;
  ...
  if(element)
    filter[element] = { $regex: search, $options: 'i' };

  return await Event.paginate(filter, { page, limit});
};
